...
`artifacts/review/bossxcode-aptitude-import-report.json` and
...
npm run aptitude:scrape-bossxcode:long

# Or tune the same long-running loop manually.
npm run aptitude:scrape-bossxcode -- --resume --max-runtime-minutes 180 --max-new-papers 500 --concurrency 2 --request-timeout 45000 --delay 1500 --checkpoint-every 10
```
...
Catalog discovery is persisted to `artifacts/aptitude-pipeline/bossxcode-catalog.json`
...
stays `bossxcode-web`, `_source.sourceProvider` is `BossXCode`, while
...
The old local PDF/OCR intake was retired because BossXCode provides structured
