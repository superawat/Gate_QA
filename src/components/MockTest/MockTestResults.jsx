...
const ResultCard = ({ label, value, accentClass }) => (
    <div className={`rounded-lg border px-3 py-2.5 text-center ${accentClass}`}>
...
const SectionSummaryCard = ({ title, summary }) => (
    <div className="rounded-lg border border-[#d6e0ea] bg-[#f8fbff] p-3">
        <div className="text-xs font-bold uppercase tracking-wide text-[#1f2f40]">{title}</div>
        <div className="mt-2 grid grid-cols-3 gap-x-3 gap-y-1.5 text-xs text-[#41576c]">
    ...
    return (
        <div className="mt-4 rounded-lg border border-[#d6e0ea] bg-[#f8fbff] p-3">
            <div className="flex items-center justify-between gap-3">
                <div>
                    <div className="text-xs font-bold uppercase tracking-wide text-[#1f2f40]">Per-question time</div>
                    <div className="mt-0.5 text-[11px] font-semibold text-[#61758a]">
                        Avg {formatMockTimeSpent(averageSeconds)} | {slowCount} over 3 min
                    </div>
                </div>
                <div className="rounded-full border border-[#f3c26f] bg-[#fff7e6] px-2.5 py-1 text-[11px] font-bold text-[#8a4b00]">
                    {formatMockTimeSpent(summary?.timeAnalysis?.totalSeconds)}
                </div>
            </div>

            <div className="mt-3 grid max-h-40 gap-1.5 overflow-y-auto pr-1">
                {rows.map((row) => {
                    const width = `${Math.max(4, Math.round((row.timeSpentSeconds / maxSeconds) * 100))}%`;
                    return (
                        <div
                            key={row.key}
                            className={`grid grid-cols-[4rem_minmax(0,1fr)_4.25rem] items-center gap-2 rounded border px-2 py-1.5 text-xs ${
                        ...
                        >
                            ...
                            <div className="h-1.5 overflow-hidden rounded-full bg-[#dce5ef]">
                                <div
...
const StructuredReviewSection = ({ questions = [], summary = {} }) => {
    ...
    return (
        <div className="mt-4 border-t border-gray-100 pt-4">
            <h3 className="text-sm font-extrabold uppercase tracking-wide text-[#1f2f40] mb-3">Structured Performance Diagnostic</h3>

            <div className="grid gap-3 md:grid-cols-2">
                {/* Left Column: Weak Topics & Mistakes */}
                <div className="space-y-4">
                    <div className="rounded-lg border border-[#d6e0ea] bg-white p-3">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#61758a] mb-2 flex items-center gap-1.5">
                        ...
                        {incorrectList.length > 0 ? (
                            <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1">
                                {incorrectList.map((iq, i) => (
                                    <div key={i} className="flex items-center justify-between gap-3 rounded border border-gray-100 bg-gray-50 px-2.5 py-2 text-xs text-[#41576c]">
                ...
                <div className="rounded-lg border border-[#d6e0ea] bg-white p-3 flex flex-col justify-between">
                    <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#61758a] mb-2">
                    ...
                    <div className="mt-3 rounded-lg border border-[#d6e0ea] bg-[#f8fbff] p-2.5">
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#61758a] mb-2">
                    ...
                    <div className="mt-3 border-t border-gray-100 pt-2.5 text-center text-xs font-bold text-[#125B9A]">
