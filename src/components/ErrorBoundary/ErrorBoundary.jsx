  ...
  return (
    <div className="rounded-[var(--radius-card)] border border-rose-200 bg-rose-50 p-6 text-center shadow-[var(--shadow-soft)]">
      ...
      <p className="mt-2 text-sm leading-6 text-rose-800">
        This section hit an unexpected error.{code ? ` (Error code: ${code})` : ""} You can try rendering it again.
      </p>
      <button
        type="button"
        onClick={reset}
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    const code = error?.code || error?.status || "";
    console.error(`[ErrorBoundary] Caught error${code ? ` (code: ${code})` : ""}:`, error, errorInfo);
  }
